        ...
        sortedMonitors.forEach((monitor) => {
          if (monitor && monitor.id) {
            initialDates[monitor.id] = {
              startDateTime: monitor.maintanance?.status
                ? dayjs(monitor.maintanance.startTime)
                : dayjs(),
              endDateTime: monitor.maintanance?.status
          ...
          >
            ...
            {monitors.length === 0 ? (
              ...
              >
                <Grid item>
                  <Typography>
                    Monitoring henüz eklenmedi, aşağıdaki düğmeye tıklayarak
                    yeni monitoring oluşturabilirsiniz.
                  </Typography>
                </Grid>
                <Grid item>
                  <Button
              ...
              <Grid container spacing={{ xs: 2, sm: 3, md: 4 }}>
                {filteredMonitors?.map((monitor) => (
                  <Grid item sx={12} md={6} xl={4} key={monitor.id}>
                    <Card
                      sx={{
                        height: 'auto',
